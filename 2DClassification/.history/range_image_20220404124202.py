...
class range_image():
    ...
    def __init__(self, filename, obj_class = None, dst_path = "./nobg_range_images/",vox = 0.01, path = "", fold = '', bg_rm = False):
        ...
        if self.bg_rm:
            self.background_subtractor = background_subtractor(self.image, PredictionConfig())
    ...
    def save_image(self, dst_path):
        ...
        car_str = "_".join(els[1:3])

        if self.obj_class == 'car':
            
            output_path = f"{dst_path}{self.obj_class}_{car_str}_{self.filename.split('.')[0]}.png" 
